...
class BillController extends Controller {
  async add() {
    ...
    try {
      ...
      // 理解为：我登陆 A 账户，那么所做的操作都得加上 A 账户的 id，后续获取的时候，就过滤出 A 账户的 id 的账单信息
  ...
  async list() {
    const { ctx, app } = this;
    // 获取 日期 date, 分页数据,类型typed_id, 都是在前端传给后端的数据
    ...
    try {
      ...
      let listMap = _list
        .reduce((curr, item) => {
          ...
          // 如果 curr 为空数组，则默认添加第一个账单项 item ,格式化为下列格式
          if (!curr.length) {
            curr.push({
              date,
              bills: [item],
  ...
  async data() {
    ...
    try {
      ...
      // _data 是经过筛选过滤出来的当月账单基础数据，每一条数据都是之前用户手动添加的，会有很多同类项。将同类项进行合并
      // 先计算总支出
